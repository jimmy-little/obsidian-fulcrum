/**
 * Embeddable Markdown Editor — full Obsidian editor (wiki links, tags, etc.) in a container.
 * Based on https://gist.github.com/Fevol/caa478ce303e69eabede7b12b2323838
 */
...
function resolveEditorBase(app: App): new (app: App, container: HTMLElement, owner: object) => EmbeddableEditorBase {
	const embedRegistry = (app as {embedRegistry?: {embedByExtension?: Record<string, (info: unknown, file: TFile, content: string) => unknown>}})
		.embedRegistry;
	const embedFn = embedRegistry?.embedByExtension?.md;
	if (!embedFn) {
		throw new Error("Fulcrum: embedRegistry.embedByExtension.md not found");
	}
	...
	if (!widgetEditorView?.editMode) {
		throw new Error("Fulcrum: Could not resolve embeddable editor prototype");
	}
...
export function createEmbeddableEditor(
	...
	const Base = editorBaseCache.get(app) ?? (() => {
	...
	class EmbeddableEditor extends Base {
		...
		constructor(
			...
			super(app, containerEl, ownerParam);
			...
			// Prevent setActiveLeaf from stealing focus when editor is focused
			workspaceUnpatch = around(app.workspace as {setActiveLeaf?: (l: WorkspaceLeaf, p?: {focus?: boolean}) => void}, {
				setActiveLeaf: (next: (l: WorkspaceLeaf, p?: {focus?: boolean}) => void) =>
					(leaf: WorkspaceLeaf, params?: {focus?: boolean}) => {
						if ((this.editor?.cm as {hasFocus?: boolean})?.hasFocus) return;
						next.call(app.workspace, leaf, params);
					},
			});

			// Focus: set activeEditor so suggests (wiki, tags) work
			...
			if (focusEl) {
				focusEl.addEventListener("focusin", () => {
					...
					app.workspace.activeEditor = ownerParam as never;
				});
	...
	const editor = new EmbeddableEditor(app, container, owner) as EmbeddableEditorBase & {getValue(): string; destroy(): void};
